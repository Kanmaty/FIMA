...
const GameResultForm = () => {
  ...
  const resetForm = (players, mode) => {
    ...
    setError("");
    setTitle("");
    // 【変更点2】シーズンはリセットせず、現在の選択を維持（または必要に応じて2026に固定）
  ...
  const validateForm = () => {
    ...
    if (activePlayers.length !== gameMode) {
      setError(`${gameMode}人麻雀の場合、順位がついている人は${gameMode}人である必要があります。`);
      return false;
    }
    ...
    <div style={{ maxWidth: "600px", margin: "0 auto", padding: "0 20px" }}>
      ...
      >
        <label style={{ marginRight: "15px", cursor: "pointer" }}>
          <input type="radio" checked={gameMode === 4} onChange={() => handleModeChange(4)} /> 4人打ち
        </label>
        <label style={{ cursor: "pointer" }}>
          <input type="radio" checked={gameMode === 5} onChange={() => handleModeChange(5)} /> 5人打ち
        </label>
      </div>

      <form onSubmit={handleSubmit}>
        {/* 【変更点5】シーズン、年月、タイトルの入力エリア */}
        <div
          style={{
            backgroundColor: "#fff",
            padding: "15px",
        ...
        >
          <div style={{ marginBottom: "15px" }}>
            <label style={{ display: "block", fontSize: "0.9em", marginBottom: "5px", fontWeight: "bold" }}>対象シーズン</label>
            <select
              value={selectedSeason}
              onChange={(e) => setSelectedSeason(e.target.value)}
              style={{ padding: "8px", width: "100%", border: "1px solid #ccc", borderRadius: "4px", backgroundColor: "#fff" }}
            >
              <option value="2026">Season2</option>
              <option value="2025">Season1</option>
            </select>
          </div>
        ...
        {/* ...（テーブル以降は変更なし）... */}
        <table style={{ width: "100%", marginBottom: "20px", borderCollapse: "separate", borderSpacing: "0 10px" }}>
          <tbody>
            {results.map((result) => (
              <tr key={result.playerId} style={{ backgroundColor: "#fff", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ textAlign: "right", marginBottom: "15px", fontWeight: "bold", fontSize: "1.1em", color: currentTotalScore !== 0 ? "red" : "green" }}>
